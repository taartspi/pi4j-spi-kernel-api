...
Userspace Raspberry Pi library for controlling SPIapi LEDs.
...
Language-specific bindings for rpi_SPIapi are available in:

* Python - https://github.com/rpi-SPIapi/rpi-SPIapi-python
* Rust - https://github.com/rpi-SPIapi/rpi-SPIapi-rust
* Powershell - https://github.com/rpi-SPIapi/rpi-SPIapi-powershell
* Java - https://github.com/rpi-SPIapi/rpi-SPIapi-java
* CSharp - https://github.com/rpi-SPIapi/rpi-SPIapi-csharp
* Go - https://github.com/rpi-SPIapi/rpi-SPIapi-go

### Background:

The BCM2835 in the Raspberry Pi has both a PWM and a PCM module that
are well suited to driving individually controllable SPIapi LEDs.
Using the DMA, PWM or PCM FIFO, and serial mode in the PWM, it's
possible to control almost any number of SPIapi LEDs in a chain connected
...
SPIapi LEDs are generally driven at 5V. Depending on your actual
...
For example, **using DMA channel 5 [will cause](https://github.com/jgarff/rpi_SPIapi/issues/224) filesystem corruption** on the Raspberry Pi 3 Model B.
...
On an RPi 4 you must set a fixed frequency to avoid the idle CPU scaling changing the SPI frequency and breaking the SPIapi timings:
...
The API is very simple.  Make sure to create and initialize the `spiApi_t`
structure as seen in [`main.c`](main.c).  From there it can be initialized
by calling `spiApi_init()`.  LEDs are changed by modifying the color in
the `.led[index]` array and calling `spiApi_render()`.
...
handler make sure to call `spiApi_fini()`.  It'll make sure that the DMA
